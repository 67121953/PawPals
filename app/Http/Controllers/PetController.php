...
class PetController extends Controller
{
    ...
    /**
     * สลับสถานะการรับเลี้ยงของสัตว์เลี้ยง (Toggle Adoption Status)
     * รองรับกับ Route: pets.toggle-adopt
     */
    public function toggleAdopt(Pet $pet)
    {
        $newStatus = $pet->adoption_status === 'available' ? 'adopted' : 'available';

        $pet->update([
            'adoption_status' => $newStatus
        ]);

        $statusText = $newStatus === 'adopted' ? 'ถูกรับเลี้ยงแล้ว' : 'พร้อมสำหรับการรับเลี้ยง';

        return redirect()
            ->back()
            ->with('success', "อัปเดตสถานะของ {$pet->name} เป็น \"{$statusText}\" เรียบร้อยแล้ว!");
    }
    ...
    private function preparePetData(Request $request, array $validated): array
    {
        $data = [
            'name' => $validated['name'],
            'type' => $validated['type'],
        ...
        if ($validated['age_unit'] === 'months') {
            $data['age_years'] = round($ageValue / 12, 4);
        } else {
